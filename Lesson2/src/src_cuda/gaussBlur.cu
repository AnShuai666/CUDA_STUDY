...
/*  @property   核函数
 *  @func       将输入图片分离为三个颜色通道
 *  @param_in   d_inputImageRGBA  输入图片
 *  @param_in   rows              图片的行数
 *  @param_in   cols              图片的列数
 *  @param_out   d_red      分离出的红色通道
 *  @param_out   d_green    分离出的绿色通道
 *  @param_out   d_blue     分离出的蓝色通道
*/
...
/*  @property   核函数
 *  @func       对单颜色通道进行高斯模糊
 *  @param_in   d_channel       单颜色通道
 *  @param_in   d_filter        高斯核
 *  @param_in   d_filterWidth   高斯核边长
 *  @param_in   rows            图片的行数
 *  @param_in   cols            图片的列数
 *  @param_out  d_channelBlurred   高斯滤波后的颜色通道
*/
__global__ void gaussblur(const unsigned char* const d_channel, unsigned char* const d_channelBlurred ,
                           float* d_filter,int rows,int cols,const int filterWidth)
{
	...
	for (int filter_y = 0; filter_y < filterWidth; filter_y++) {
		for (int filter_x = 0; filter_x < filterWidth; filter_x++) {
			...
			color += filter_value * static_cast<float>(d_channel[image_y *cols + image_x]);
		}
	}
	d_channelBlurred[idx] = color;

}

/*  @property   核函数
 *  @func       合并高斯模糊后的三条颜色通道
 *  @param_in   d_redBlurred    高斯滤波后的红色通道
 *  @param_in   d_greenBlurred  高斯滤波后的绿色通道
 *  @param_in   d_blueBlurred   高斯滤波后的蓝色通道
 *  @param_in   rows            图片的行数
 *  @param_in   cols            图片的列数
 *  @param_out  d_outputImageRGBA   合并通道后的图片
*/
